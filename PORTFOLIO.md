...
| **자체 성분 DB** | 주요 30개 성분의 검증된 과학 데이터를 AI 프롬프트에 주입 (RAG 방식) |
| **식약처 MFDS API** | 2만 개+ 화장품 원료성분 공공데이터 실시간 검증 |

> **RAG 패턴 적용**: AI가 자체 지식만으로 답변하면 Hallucination이 발생. 자체 성분 DB + 식약처 API 데이터를 프롬프트에 주입하여 **검증된 데이터 기반 답변**을 유도하는 Retrieval-Augmented Generation 패턴 적용.
...
1. **AI 프롬프트 엔지니어링 (RAG)**: "지어내지 마"보다 "이 데이터만 써"가 100배 효과적. 검증된 데이터를 프롬프트에 직접 주입하는 RAG 방식이 Hallucination 방지의 핵심.
