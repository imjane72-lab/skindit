...
### 6-16. 검증 없는 데이터가 서비스 신뢰도를 훼손

**문제**: 히어로 카피에 "10만 개 성분 데이터 기반"이라고 표기했으나, 실제 식약처 MFDS API의 등록 성분 수는 **21,696개**. 약 5배 과장된 수치.

**발견 경위**: 포트폴리오 작성 중 실제 API를 호출하여 `totalCount`를 확인 → 10만이 아닌 2만임을 발견

**해결**: 모든 곳에서 "10만 개" → "2만 개+"로 수정 (히어로, 메타 태그, 푸터, 포트폴리오)

**교훈**: 마케팅 카피라도 **실제 데이터와 일치해야 한다**. 특히 기술 기반 서비스에서 수치를 과장하면 개발자/투자자 리뷰에서 신뢰를 잃는다. 배포 전 주요 수치는 반드시 검증할 것.
