...
| 자체 성분 DB | 주요 28개 성분의 검증된 과학 데이터를 프롬프트에 직접 주입 |
...
> **RAG 고도화 4가지 원칙**
> 1. **하이브리드 검색** — 자체 DB(키워드 매칭) + MFDS API(공공데이터) 병렬 조회
> 2. **가드레일** — "컨텍스트에 없는 내용은 출력 차단"
> 3. **출처 표기** — "식약처 등록 성분이에요"처럼 출처 명시
...
flowchart LR
    ...
    subgraph PARALLEL["⚡ 병렬 조회"]
        SELF[("자체 성분 DB<br/>28개 검증 데이터")]:::src
...
- **3단계 해결**
  1. 주요 28개 성분의 과학적 사실을 `ingredient-db.ts`에 하드코딩하고 프롬프트에 직접 주입 (RAG)
...
1. **RAG가 만능이 아니라, "어떻게 RAG를 짜느냐"가 만능이다** — 하이브리드 검색 + 가드레일 + 출처 표기 + No-Answer 4가지가 같이 가야 효과가 났습니다.
