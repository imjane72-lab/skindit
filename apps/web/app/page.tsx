  ...
  const loadTrendInfo = async (id: string, name: string) => {
    ...
    try {
      const raw = await callAIText(
        `스킨케어 성분 전문가. 반존대. ${lang === "ko" ? "한국어" : "English"}. 확실한 정보만. 추측 금지. 모르면 안 쓰기. ~ 물결표시 거의 쓰지 마세요. "절대 금지"라는 표현 쓰지 마세요. 불활성 성분에 없는 성질 만들지 마세요.`,
        `"${name}" 성분 가이드. 각 항목 1-2줄:

**작용** 피부에서 하는 일 (확실한 메커니즘만)
**사용 시간** 아침/저녁 + 이유
**주의 콤보** 같이 쓸 때 주의할 성분 (AHA+레티놀 같은 검증된 것만. 없으면 "특별한 주의 콤보 없음")
**시너지** 같이 쓰면 좋은 성분 2개 + 이유
**꿀팁** 실전 팁 2개
**효과 시기** 언제부터?
**추천** ★(1-5) + 어떤 피부에 좋은지`
      )
  ...
  const runCompareAnalysis = async (
    ...
    setPhase("loading")
    const sys = `너는 skindit 언니야. 두 제품 비교해서 친한 동생한테 알려주듯이 말해줘. 지어내기 금지, 입력 성분만 비교.
⚠️ 절대 규칙: 데이터에 있는 성분만 언급. 등록 여부 데이터가 없으면 등록 여부를 아예 언급하지 마세요.
⚠️ 과학적 정확성 필수:
- "절대 금지"라는 표현 쓰지 마세요. "주의", "분리 사용 권장" 등으로 쓰세요.
- 불활성 성분(실리카, 정제수 등)에 햇빛 반응 같은 없는 성질을 만들어내지 마세요.
- 확실하지 않은 메커니즘(수용체 이름, 상호작용)은 쓰지 마세요.
- 금지 콤보는 AHA+레티놀 등 실제 검증된 것만. 추측 금지.
- 민감성 피부라도 "절대 금지"가 아니라 "간격 두고 사용" 또는 "아침/저녁 분리" 권장으로 쓰세요.

핵심: 성분 하나하나 설명 말고, 제품 단위로 꿀팁을 알려줘!
- 이 제품은 아침/저녁 중 언제 쓰면 좋은지 + 이유
- 두 제품을 같이 쓸 때 순서 (예: A 먼저 → B 나중에)
- 두 제품 같이 쓰면 시너지가 나는지, 주의할 점은 없는지
- 제형이 같으면(세럼+세럼) 하나만 추천, 다르면(세럼+크림) 같이 쓰는 꿀팁
- 초보자가 알아야 할 주의사항 2-3개
- 효과 체감 시기

⚠️ verdict에서 반드시: 과학적 근거 포함 추천 이유 구체적으로.

JSON only. Schema:{"summary":"3-4 sentences, 두 제품 핵심 비교 + 같이 쓰면 어떤지, 반존대","shared":[max 5,{"name":"","inA":true,"inB":true,"note":"겹치는데 괜찮은지+사용팁"}],"only_a":[max 5,{"name":"","inA":true,"inB":false,"note":"왜 좋은지+사용팁"}],"only_b":[max 5,{"name":"","inA":false,"inB":true,"note":"왜 좋은지+사용팁"}],"forbidden_combos":[max 2,{"ingredients":"A + B","reason":"왜 안 되는지 과학적으로"}],"recommendation":"어떤 피부에 뭐가 맞는지 3-4 sentences","usage_guide":{"best_time":"A 제품: 아침/저녁 + 이유, B 제품: 아침/저녁 + 이유, 같이 쓸 때 순서","effect_timeline":"각 제품별 효과 체감 시기","beginner_tips":["꿀팁 3개 - 실용적이고 구체적으로"]},"verdict":"추천 강도 ★(1-5) + 과학적 근거 포함 추천 이유 3-4 sentences"}. ${useLang === "ko" ? "한국어" : "English"}.`
    const skinContext =
...
말투: 친근하지만 신뢰감 있는 존댓말. ~ 물결표시 거의 쓰지 마세요. "좋아요", "바르세요", "있어요" 등 깔끔한 존댓말로.
...
⚠️ concern_analysis에서 사용자가 선택한 고민을 반드시 전부 포함해주세요. 하나도 빠뜨리면 안 됩니다.

JSON only. Schema:{"overall_score":0-100,"overall_comment":"2-3 sentences, 반존대","concern_analysis":[사용자가 선택한 고민 전부,{"concern":"","score":0-100,"comment":"꿀팁+주의사항, 3줄 이상 상세하게"}],"star_ingredients":[max 5,{"name":"","benefit":"왜 좋은지+사용팁","best_time":"아침/저녁/둘다","synergy":["시너지 성분"]}],"watch_out":[{"name":"","reason":"왜 주의+꿀팁","alternative":"대신 이거~"}],"forbidden_combos":[max 3,{"ingredients":"A + B","reason":"왜 안 되는지"}],"usage_guide":{"best_time":"아침/저녁/둘다 + 이유","effect_timeline":"효과 체감까지 걸리는 시간","beginner_tips":["초보자 주의사항 3개"]},"safety_ratings":[max 8,{"name":"","score":1-10,"note":"꿀팁"}],"verdict":"추천 강도 ★(1-5) + 과학적 근거 포함 추천 이유 3-4 sentences"}. watch_out: allergens/irritants only. safety: 1=safe,10=hazard. ${useLang === "ko" ? "한국어" : "English"}.`
    const skinContext =
      profileSkinTypes.length > 0
      ...
      {phase === "setup" && (
        <div className="relative overflow-hidden border-b border-gray-100/50 px-6 pt-14 pb-12">
          ...
          <div className="relative">
            ...
            >
              {t("10만 개 성분 데이터 기반 AI 분석", "AI Analysis Powered by 100K+ Ingredients")}
            </p>
        ...
        <footer className="border-t border-gray-100 bg-gray-50/50 px-6 py-10">
          <div>
            ...
            <div className="border-t border-gray-100 pt-4 text-[10px] text-gray-300">
              <p>© 2026 skindit. {t("10만 개 성분 데이터 기반 AI 스킨 분석", "AI skin analysis powered by 100K+ ingredient data")}</p>
            </div>
          </div>
        </footer>
      )}
