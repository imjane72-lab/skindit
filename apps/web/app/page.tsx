      ...
      {phase === "setup" && (
        <div className="relative overflow-hidden border-b border-gray-100/50 px-6 pt-14 pb-12">
          ...
          <div className="relative">
            ...
            >
              {t(
                "성분 분석부터 피부 일지, 트러블 추적까지 — 다 해주는 내 피부 메이트 💜",
            ...
            >
              <p className="mb-2 text-center text-[10px] font-bold tracking-wider text-gray-300 uppercase">
                {t("👇 아래에서 바로 분석해보세요~", "👇 Try analyzing below~")}
              </p>
              <div className="flex flex-wrap justify-center gap-2">
                {[
                  { label: t("단일 성분 분석", "Single Analysis"), icon: "💊" },
                  { label: t("루틴 궁합 체크", "Routine Check"), icon: "🧴" },
                  { label: t("성분 비교", "Compare"), icon: "⚖️" },
                  { label: t("사진 스캔", "Photo OCR"), icon: "📷" },
                ].map((f, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-1 rounded-full border border-gray-100 bg-white/80 px-3 py-1.5 text-[11px] font-semibold text-gray-500 shadow-sm"
                  >
